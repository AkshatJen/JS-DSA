...
let filteredArray = arr.filter(
  (x) => typeof parseInt(x) == "number" && x % 2 == 1
);
...
console.log(
  "============mapped array of square of odd numbers================= \n"
);
...
console.log(
  "=======reduced array of addition of squares of odd numbers======== \n"
);
console.log(reducedArray + "\n\n");

// using chaining method can also help us achieve similar results
// code above is equivalent to the code below
let filteredMapReducedArray = arr
  .filter((x) => typeof parseInt(x) === "number" && x % 2 === 1)
  .map((x) => x * x)
  .reduce((x, y) => x + y);
